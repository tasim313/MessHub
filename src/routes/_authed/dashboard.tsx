    ...
    () =>
      members.find(
        (member) =>
          ...
          (!!profile?.email && member.email === profile.email),
      ),
    [members, profile],
  );
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-6">
        {profileError && (
          <Card className="border-amber-500/40 bg-amber-500/5 p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Dashboard loaded, but Firebase profile sync had an issue:{" "}
              {profileError}
            </p>
          </Card>
        )}
        ...
        >
          <StatCard
            label={isOwnerOrManager ? "Current Balance" : "My Balance"}
            value={bdt(isOwnerOrManager ? balance : mySummary?.balance || 0)}
            icon={Wallet}
            tone={
              (isOwnerOrManager ? balance : mySummary?.balance || 0) >= 0
                ? "primary"
                : "danger"
            }
            hint={
          ...
          <StatCard
            ...
            icon={TrendingUp}
            hint={
              isOwnerOrManager ? "Bazar + utilities + salary" : "This month"
            }
          />
          <StatCard
            label={isOwnerOrManager ? "Monthly Meals" : "My Meals"}
            value={String(
              isOwnerOrManager ? summary.totalMeals : mySummary?.meals || 0,
            )}
            icon={Utensils}
            tone="primary"
            hint={`Meal rate ${bdt(summary.mealRate)}`}
          />
          <StatCard
            label={isOwnerOrManager ? "Active Members" : "Utility Share"}
            value={
              isOwnerOrManager
                ? String(members.filter((member) => member.active).length)
                : bdt(mySummary?.utilityShare || 0)
            }
            icon={isOwnerOrManager ? Users : BellRing}
            hint={
              isOwnerOrManager
                ? `${dueMembers.length} members due`
                : "This month"
            }
          />
        </motion.div>
        ...
        {isOwnerOrManager && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              ...
              icon={BedDouble}
              hint={`${summary.occupiedBeds} occupied`}
            />
            <StatCard
              label="Salary cost"
              value={bdt(summary.totalStaffCost)}
              icon={Users}
              hint={`${staff.filter((item) => item.status !== "inactive").length} active staff`}
            />
            <StatCard
              label="Rent ledger"
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Owner can monitor every member dashboard summary
                </p>
              </div>
              <Button asChild variant="ghost" size="sm">
                <Link to="/members">
                  Manage members <ArrowUpRight className="ml-1 h-3.5 w-3.5" />
                </Link>
              </Button>
            </div>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {summary.perMember.map((item) => (
                  <Card key={item.memberId} className="p-4">
                    ...
                    <div
                      className={`mt-4 rounded-lg px-3 py-2 text-sm font-semibold ${item.balance >= 0 ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
                    >
                      {item.balance >= 0
                        ? "Advance balance"
                        : "Outstanding due"}
                      : {bdt(Math.abs(item.balance))}
                    </div>
                  </Card>
                ))}
              </div>
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Personal meal, deposit, and due details
                </p>
              </div>
            ...
            {!currentMember || !mySummary ? (
              <p className="text-sm text-muted-foreground py-6 text-center">
                Your account is signed in, but no linked member profile was
                found yet. Ask the owner to update your member email or uid.
              </p>
            ) : (
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <Card className="p-4">
                  ...
                  <div
                    className={`mt-2 text-2xl font-bold ${mySummary.balance >= 0 ? "text-primary" : "text-destructive"}`}
                  >
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Latest shared expense activity
              </p>
            </div>
            <Button asChild variant="ghost" size="sm">
              <Link to="/bazar">
                ...
                >
                  <div>
                    <div className="font-medium text-sm">
                      {item.category || "Bazar"} · {item.buyerName}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {item.date}
                    </div>
