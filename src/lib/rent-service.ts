...
/**
 * Ensure every active, room-assigned member has a rent charge for every
 * month from when they joined through the current month. Pass in the
 * already-loaded rent_charges and ledgers collections (from useCollection)
 * so this only ever needs to run the in-memory existence checks plus the
 * actual writes — no extra reads.
 */
export async function ensureRentChargesUpToDate(
  ...
  const activeMembers = members.filter((m) => m.active && m.roomId);
  if (activeMembers.length === 0) return { created: 0, months: [] };
    ...
    for (const ym of months) {
      ...
      if (!hasLedgerEntry) {
        ...
        nextRef().set(ledgerRef, {
          ...
          category: "rent",
          amount: rentAmount,
          notes: `Rent for ${ym} - ${room.roomNo} (auto-generated)`,
